...
numbers refer to commit `{SHA[:7]}`. Each function link is followed by
...
`main()` in [`map_builder.rs`]({link(map_rs, 989)}) so you can step
through the driver source in parallel.

The toy simulation spans {g3_num_sites} VCF sites. Everything below
...
- **Spouse**, a founder, is handed the fresh letter pair `(E, F)` by
  ...
  [`map_builder.rs:1059`]({link(map_rs, 1059)}) for the master
  template and [`map_builder.rs:1111`]({link(map_rs, 1111)}) per VCF
...
`(C, D)` — static founder pairs that [`Iht::new`]({link(iht_rs, 172)})
pre-filled at startup. In the second it reads *Kid3's* slot pair,
which [`track_alleles_through_pedigree`]({link(map_rs, 295)}) itself
wrote during the first triple. [`get_iht_markers`]({link(map_rs, 274)})
...
[`summarize_child_changes`]({link(map_rs, 673)}) (driver call at
[`map_builder.rs:1228`]({link(map_rs, 1228)})). But that rule does
...
def component_3_concordance(out_dir: Path) -> None:
    ...
    body_2 = [
        ...
        "  2. For each kid, deduce the expected phased genotype given",
        "     the chosen letter->allele mapping (e.g. under A=0, C=1,",
        "     the phased letter pair A|C becomes the phased genotype 0|1).",
...
alongside two phased output VCFs: `{prefix}.pass.vcf` for records the
block labels fully explain, and `{prefix}.fail.vcf` for records they
...
PAGE_CHOICES = ["nuclear_family", "three_generations", "concordance"]
